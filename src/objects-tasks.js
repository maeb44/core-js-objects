...
/** The body of a for-in should be wrapped in an if statement to filter unwanted properties from the prototyp
...
function makeWord(lettersObject) {
  ...
  Object.values(lettersObject).forEach((cur, index) => {
    cur.forEach((el) => {
      arr[el] = keys[index];
    });
  });
  ...
  queue.forEach((el) => {
    switch (el) {
      case 25:
        ...
        break;
      case 50:
        if (seller.change < 25) bool = false;
        seller.change += 25;
        break;
      case 100:
        if (seller.change < 75) bool = false;
        seller.change += 75;
        break;
      default:
        bool = false;
...
class CssSelector {
  ...
  isAfter() {
    ...
    const lastIndex = order.indexOf(this.last);
    const curIndex = order.indexOf(this.type);
  ...
  stringify() {
    ...
    if (this.attr1.length > 0)
      this.attr1.map((el) => {
        result += `[${el}]`;
        return el;
      });
    if (this.pseudoClass1.length > 0)
      result += `:${this.pseudoClass1.join(':')}`;
